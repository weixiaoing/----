<template>
  <div class="chat div-wrap">
    ...
    >
      <ul>
        <li
          ...
          :key="index"
          :class="{
            'chat-right': item.chatWith._id == chatWithId,
        ...
        >
              ...
              index == 0 ||
                (dataList[index - 1] &&
                  new Date(item.addTime) -
                    new Date(dataList[index - 1].addTime) >
                    1000 * 180)
            "
          >
          ...
          <div
            :class="{ toright: item.chatWith._id == chatWithId, flex: true }"
          >
    ...
    <div class="chat-footer">
      <textarea
        class="chat-textarea weui-textarea"
        v-model="content"
        rows="1"
      ></textarea>
...
export default {
  ...
  methods: {
    ...mapMutations(["update_chatList"]),
    //获取聊天对象信息
    getUserInfo() {
      this.axios
        .get(`/profile/userInfo?name=${this.chatWith}`)
        .then(({ data }) => {
          if (data) {
            this.tUserInfo = data;
          }
